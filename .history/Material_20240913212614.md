...
在DogItem()内部，添加Column布局，使用animateContentSize修饰符链接符该修饰符，以便为大小（列表项高度）变化添加动画效果。
...
现在动画会缓入和缓出，不会有弹跳。因此，此时animationSpec形参添加到animateContentSize()函数调用中。DampingRatioNoBouncy将其设置为弹簧动画，产生无弹跳，并使用StiffnessMedium形参让弹簧略硬。
    ...
    modifier = Modifier
        .animateContentSize(//添加动画效果
            animationSpec = spring(
                dampingRatio = Spring.DampingRatioNoBouncy,//弹簧动画
                stiffness = Spring.StiffnessMedium//弹簧略硬
            )
        )
    )
...
## 使用animateColorAsState()函数更改列表项中展开状态时的颜色

在DogItem()中，声明一种颜色放置其初始化委托给animateColorAsState()函数。根据expanded布尔值，设置targetValue具名形参。如果列表项设置为颜色，则接受列表项设置为tertiaryContainer颜色。否则，支持其设置为primaryContainer颜色。
